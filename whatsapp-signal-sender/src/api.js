...
async function sendTo(phoneOrContact, message) {
    ...
    try {
        ...
        process.stdout.write(`https://web.whatsapp.com/send?phone=${phone}&text=${encodeURIComponent(message)}`)
        await page.waitForSelector(SELECTORS.LOADING, { hidden: true, timeout: 15000 });
        await page.waitForSelector(SELECTORS.SEND_BUTTON, { timeout: 20000 });
...
async function sendToGroup(phoneOrContact, mensagem_sinal) {
    ...
    try {
        ...
        // // await page.click('//*[@id="main"]/footer/div[1]/div/span[2]/div/div[2]/div[1]/div/div[1]/p');
        // // await page.keyboard.type('Seu texto aquwwi');
        // await page.waitFor(5000);
        // await page.keyboard.type('Seu texto aquwwi');
        // await page.type('#main footer div.selectable-text[contenteditable]', 'Seu texto aqui');
        await page.keyboard.press('Enter');
        // process.stdout.write(`https://web.whatsapp.com/send?phone=${phone}&text=${encodeURIComponent(message)}`)
        // await page.waitForSelector(SELECTORS.LOADING, { hidden: true, timeout: 15000 });
        // await page.waitForSelector(SELECTORS.SEND_BUTTON, { timeout: 20000 });
        ...
        // process.stdout.clearLine();
        // process.stdout.cursorTo(0);
        process.stdout.write(`Enviado para o Grupo.\n`);
        await page.waitFor(5000);
        counter.success++;
    } catch (err) {
        process.stdout.clearLine();
        process.stdout.cursorTo(0);
        process.stdout.write(`Envio da mensagem falhou\n`);
        counter.fails++;
    }
}
...
async function end() {
    await browser.close();
    console.log(`Resultado: ${counter.success} enviado, ${counter.fails} erro`);
}
